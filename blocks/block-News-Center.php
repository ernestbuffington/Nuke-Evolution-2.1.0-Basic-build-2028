    ...
    if ($artinfo['aid'] == $informant) {
        ...
        if($informantwrites == 0) {
            if(!empty($informant)) {
                ...
                $content = $lang_block['BLOCK_NEWS_ANONYMOUS'];
            }
            $content .= $lang_block['BLOCK_NEWS_WRITES']." \"<em>".$artinfo['hometext']."\"</em>".$notes;
        } else {
            $content .= $artinfo['hometext'].$notes;
        }
    }
